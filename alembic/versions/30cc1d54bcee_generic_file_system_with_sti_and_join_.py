...
down_revision: Union[str, None] = 'b5bf6597608c'
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['uploaded_by_id'], ['users.id'], name=op.f('fk_files_uploaded_by_id_users')),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['file_id'], ['files.id'], name=op.f('fk_location_files_file_id_files'), ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['location_id'], ['user_locations.id'], name=op.f('fk_location_files_location_id_user_locations'), ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('location_id', 'file_id', name=op.f('pk_location_files')),
    )
    ...
    op.create_table(
        ...
        sa.Column('file_id', sa.Uuid(), nullable=False),
        sa.Column('added_at', sa.DateTime(), server_default=sa.text('now()'), nullable=True),
        sa.ForeignKeyConstraint(['file_id'], ['files.id'], name=op.f('fk_scouting_files_file_id_files'), ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['scouting_id'], ['scoutings.id'], name=op.f('fk_scouting_files_scouting_id_scoutings'), ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('scouting_id', 'file_id', name=op.f('pk_scouting_files')),
    )
